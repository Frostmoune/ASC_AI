...
class Vis_lstm_model:
	...
	def __init__(self, options):
		with tf.device('/gpu:0'):
			...
			self.Wimg = self.init_weight(options['feature_length'], options['embedding_size'], name = 'Wimg')
	...
	def build_model(self):
		features = tf.placeholder('float32',[ None, self.options['feature_length'] ], name = 'features')
		...
		image_embedding = tf.matmul(features, self.Wimg) + self.bimg
		image_embedding = tf.nn.relu(image_embedding + 1e-12)
